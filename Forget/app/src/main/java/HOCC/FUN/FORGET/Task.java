...
public class Task extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SharedPreferences pref=this.getSharedPreferences("NE", MODE_PRIVATE);
        if (pref.getInt("Size", 0) == 1){
            stopService();
            size = pref.getInt("Size" , 0);
            serviceIntent.putExtra("size" , size);
            this.getSharedPreferences("NE", MODE_PRIVATE).edit().putInt("Size", 1).apply();
